...
namespace MouseBound
{
    public class MonitorChanger
    {
        public static void SetAsPrimaryMonitor(string deviceName)
        {
            ...
            NativeMethods.EnumDisplaySettings(deviceName, -1, ref deviceMode);
            ...
            {
                if (device.StateFlags.HasFlag(DisplayDeviceStateFlags.AttachedToDesktop) && device.DeviceName !=deviceName)
                {
                    device.cb = Marshal.SizeOf(device);
